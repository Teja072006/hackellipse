...
const LearningMilestoneSchema = z.object({
  ...
  quiz: z.array(QuizQuestionSchema).max(5).optional().describe("An optional short quiz of 3-5 multiple-choice questions specific to this milestone's content. Each question should have 'questionText', 'options' (array of 4 strings), 'correctAnswerIndex' (0-3), and an optional 'explanation'.")
});
...
const GenerateLearningPlanOutputSchema = z.object({
  ...
  milestones: z.array(LearningMilestoneSchema).min(3).max(7).describe('An array of learning milestones, typically 3 to 7 milestones, ordered logically to guide the user from basics to more advanced topics.'),
});
...
The plan should consist of several logical milestones (3 to 7), progressing from foundational concepts to more advanced topics or practical application.
...
6.  Optionally, a short "quiz" of 3-5 multiple-choice questions directly related to the content and objectives of THIS milestone. Each quiz question must have:
    ...
    Ensure quiz questions are clear, relevant, and test understanding of the milestone's key concepts. The quiz array should have a maximum of 5 questions.
...
- "milestones": An array of 3 to 7 logically ordered milestones.
    ...
    output.milestones.forEach((m, index) => {
        if (m.quiz && m.quiz.length > 0) {
            if (m.quiz.some(q => !q.questionText || !q.options || q.options.length !== 4 || q.correctAnswerIndex === undefined || q.correctAnswerIndex < 0 || q.correctAnswerIndex > 3)) {
                console.warn(`[generateLearningPlanFlow Genkit] Milestone ${index + 1} has a malformed quiz. Raw milestone:`, m);
