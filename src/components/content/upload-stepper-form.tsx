...
const formSchema = z.object({
  ...
  if (data.contentType === "video") {
    ...
    if (!hasFile && !hasTextBody) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Either upload a text file or enter text content directly.", path: ["file"] }); // path can be file or textContentBody
    }
...
export function UploadStepperForm() {
  ...
  const [isUploadingFile, setIsUploadingFile] = useState(false); // Specific for file upload phase
  const [isSavingToDB, setIsSavingToDB] = useState(false); // Specific for DB save phase
  ...
  const processToNextStep = async () => {
    ...
    if (currentStep === 1) {
      ...
      allFieldsValid = await form.trigger();
      if (!allFieldsValid) {
      ...
      if (currentFile) {
        if (currentFile.size > MAX_FILE_SIZE_FOR_CLIENT_AI) {
          toast({
            title: "AI Processing Skipped",
            description: `File size (${(currentFile.size / (1024 * 1024)).toFixed(2)}MB) is over ${MAX_FILE_SIZE_FOR_CLIENT_AI / (1024 * 1024)}MB for client-side AI analysis. Description will use your manual input if provided.`,
            variant: "default",
            duration: 7000,
          });
          skipAI = true;
          ...
          try {
            ...
            toast({ title: "File Read Error", description: `Could not read file for AI processing: ${e.message}`, variant: "destructive" });
            skipAI = true;
          }
        }
        ...
        if (new TextEncoder().encode(values.textContentBody).length > MAX_FILE_SIZE_FOR_CLIENT_AI) {
          toast({
            title: "AI Processing Skipped",
            description: `Text content is too large for client-side AI analysis. Description will use your manual input if provided.`,
            variant: "default",
            duration: 7000,
          });
          skipAI = true;
        } else {
          // Ensure correct Base64 encoding for text data
          dataUriForAI = `data:text/plain;base64,${typeof window !== "undefined" ? window.btoa(unescape(encodeURIComponent(values.textContentBody))) : Buffer.from(values.textContentBody).toString('base64')}`;
        }
      }

      if (skipAI) {
        setAiResult({ isValid: true, description: values.user_manual_description || "AI processing skipped. Please add/edit description if needed." });
        ...
        try {
          ...
          setAiResult({ isValid: true, description: values.user_manual_description || "AI processing failed. Please add/edit description if needed." });
        }
      } else { // No file and no text content, or some other case for not processing with AI
        setAiResult({ isValid: true, description: values.user_manual_description || "No content provided for AI analysis. Please add/edit description if needed." });
      }
  ...
  const onSubmit = async (data: UploadFormValues) => {
    ...
    console.log("File selected for storage:", fileForStorage ? fileForStorage.name : "No file for storage");
    ...
    try {
        ...
        await new Promise<void>((resolve, reject) => {
          uploadTask.on("state_changed",
            ...
            (error) => {
              ...
              setUploadError(`Storage Upload Error: ${error.message} (Code: ${error.code})`);
            ...
            async () => {
              ...
              try {
                ...
                setIsUploadingFile(false); // Mark file upload phase as complete
                resolve();
              } catch (getUrlError: any) {
                 console.error("Error getting download URL:", getUrlError);
                 setUploadError(`Error getting download URL: ${getUrlError.message}`);
                 toast({ title: "Upload Error", description: "Could not get file URL after upload.", variant: "destructive" });
                 setIsUploadingFile(false);
                 setIsSubmitting(false);
                 reject(getUrlError);
              }
         ...
         console.log("No file to upload to Firebase Storage (e.g., direct text input).");
      }

      // Proceed to save metadata to Firestore
      setIsSavingToDB(true);
      console.log("Saving metadata to Firestore...");

      const batch = writeBatch(db);

      // 1. Create master content document in 'content_types' (Firestore)
      const contentTypesCollectionRef = collection(db, "content_types");
      const newContentTypeRef = doc(contentTypesCollectionRef); // Auto-generate ID
      ...
      const contentDocPayload = {
        ...
        average_rating: 0,
        total_ratings: 0,
        // Use AI description if available, otherwise manual, otherwise a default.
        brief_summary: aiResult?.description || data.user_manual_description || "No summary available."
      };
      batch.set(newContentTypeRef, contentDocPayload);
      const contentId = newContentTypeRef.id;
      console.log("Prepared content_types document for Firestore with ID:", contentId);


      // 2. Create specific content document (videos, texts, audios)
      ...
      if (specificContentCollectionName) {
        ...
        if (data.contentType === "video") {
          specificContentPayload.video_path = fileDownloadURL; // From Firebase Storage
          specificContentPayload.duration_seconds = null; // Placeholder, needs actual calculation
        } else if (data.contentType === "audio") {
          specificContentPayload.audio_path = fileDownloadURL; // From Firebase Storage
          specificContentPayload.duration_seconds = null; // Placeholder
        } else if (data.contentType === "text") {
          if (finalStoragePath && fileDownloadURL) { // If a text file was uploaded to Storage
            ...
            // Store smaller text content directly, larger ones might need different strategy
        ...
        console.log(`Prepared ${specificContentCollectionName} document for Firestore.`);
      }
      ...
      setFileName(null);
      setFileToUpload(null);
      setUploadProgress(null); // Reset progress for next upload

    } catch (error: any) {
      console.error("Error submitting content to Firestore or during final steps:", error);
      ...
      setIsUploadingFile(false);
      setIsSavingToDB(false);
      // Do not reset uploadProgress here if you want it to show 100% on success before form reset
    }
  };

  const overallLoading = isProcessingAI || isSubmitting || isUploadingFile || isSavingToDB;
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              ...
              <>
                ...
                {watchedContentType === "text" && (
                  <Alert variant="default" className="bg-secondary/20 border-secondary/40">
                    ...
                    <AlertDescription>
                      You can either upload a text file (e.g., .txt, .md, .pdf, .docx up to {MAX_FILE_SIZE_TEXT_FILE / (1024 * 1024)}MB)
              ...
              <div className="space-y-6">
                ...
                {uploadProgress !== null && (
                  <div className="space-y-1">
                      <Label className="text-primary">{isSavingToDB ? "Finalizing metadata..." : (uploadProgress < 100 ? `Uploading to SkillForge: ${Math.round(uploadProgress)}%` : "Upload complete! Finalizing...")}</Label>
          ...
          <CardFooter className="flex justify-between border-t border-border pt-6">
            <Button type="button" variant="outline" onClick={() => setCurrentStep(s => s - 1)} disabled={overallLoading || currentStep === 1} className="hover:border-primary hover:text-primary">
              <ArrowLeft className="mr-2 h-4 w-4" /> Previous
            </Button>

            {currentStep < 3 && (
              <Button type="button" onClick={processToNextStep} disabled={overallLoading || !watchedContentType || (currentStep === 2 && (!form.formState.isValid && !isProcessingAI))} className="ml-auto bg-primary hover:bg-accent">
                {(isProcessingAI && currentStep === 2) ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
                Next <ArrowRight className="ml-2 h-4 w-4" />
              </Button>
            )}
            {currentStep === 3 && (
              <Button type="submit" disabled={overallLoading || isProcessingAI} className="ml-auto bg-green-600 hover:bg-green-700 text-white">
                {isSubmitting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <CheckCircle className="mr-2 h-4 w-4" />}
                {isSubmitting ? (isUploadingFile ? 'Uploading file...' : (isSavingToDB ? 'Saving metadata...' : 'Processing...')) : "Submit to SkillForge"}
              </Button>
            )}
          </CardFooter>
        </form>
